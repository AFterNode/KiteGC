package net.optifine.Gui.LiquidBaseClickGui.UI;
...
public class ClickGUI extends GuiScreen {
    ...
    private List<net.optifine.Gui.LiquidBaseClickGui.UI.Panel> panels = new ArrayList<>();

    public ClickGUI() {
        int x = 20;

        for(final ModuleType moduleCategory : ModuleType.values()) {
            final net.optifine.Gui.LiquidBaseClickGui.UI.Panel panel = new net.optifine.Gui.LiquidBaseClickGui.UI.Panel(moduleCategory.getName(), x, 50, 100);
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        for(final net.optifine.Gui.LiquidBaseClickGui.UI.Panel panel : panels) {
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        if(mouseButton == 0) {
            for(int index = panels.size() - 1; index >= 0; index--) {
                final net.optifine.Gui.LiquidBaseClickGui.UI.Panel panel = panels.get(index);
